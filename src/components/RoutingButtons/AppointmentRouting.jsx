    ...
    <div className="mt-16">
      {navValues.map((item) => (
        <Link
          key={item.name}
          to={item.path}
          className="md:px-6 md:py-2 sm:px-2 sm:py-2 bg-gray-600 text-white rounded-md hover:bg-yellow-400 md:m-10 sm:m-2 focus:ring-4 focus:ring-blue-300 font-semibold text-sm dark:bg-yellow-600 dark:hover:bg-yellow-700 focus:outline-none dark:focus:ring-yellow-800 hover:font-bold border-2 border-gray-200 hover:text-gray-900"
        >
          {item.name}
        </Link>
      ))}
